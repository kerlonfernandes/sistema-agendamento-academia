...
class Helpers
{
    ...
    function decodeURL($str)
    {
        $str = strtr(
        string: $str,
        from: "a8rqxPtfiNOlYFGdonMweLCAm0TXERcugBbj79yDVIWsh3Z5vHS46pQzKJ1Uk2", 
    ...
    public function compareProximityBetweenLists(array $list1, array $list2)
    {
        $bestMatch = '';
        $bestPercentual = 0;
        ...
        foreach ($list1 as $str1) {
            foreach ($list2 as $str2) {
                similar_text($str1, $str2, $percentual);

                if ($percentual > $bestPercentual) {
                    $bestPercentual = $percentual;
                    $bestMatch = $str2;
                }
            }
        }
    ...
    public function compareProximityBetweenListsAndReturnList(array $list1, array $list2): array
    {
        $bestMatches = [];
        ...
        foreach ($list1 as $str1) {
            ...
            $bestMatches[$str1] = $bestMatch;
        }

        return $bestMatches;
    }

    public function mountTxtRow($antenna, $athleteNum, $athleteTime) {
        
        $row ="";
        
        if($antenna) {
            $formattedAntennaNum = str_pad($antenna, 3, '0', STR_PAD_LEFT);
            $row .= $formattedAntennaNum;
        }

        $row .= "000000000000";
        
        if ($athleteNum) {
            $formattedAthleteNum = str_pad($athleteNum, 4, '0', STR_PAD_LEFT);
            $row .= $formattedAthleteNum;
        }
